...
connect_ais_stream <- function(api_key, bounding_box = NULL, file_path = NULL, layer = NULL, verbose = TRUE, reconnect_delay = 5, heartbeat_interval = NULL, outDir = NULL) {
  ...
  # --- New: Handle file input and check for both bounding_box and file_path ---
  ...
  # --- NEW: Geopspatial filtering section ---
  ais_polygon <- NULL
  ...
  if (!is.null(file_path)) {
    ...
    # Calculate the bounding box from the polygon and assign it to bbox_api
    bbox_sf <- sf::st_bbox(ais_polygon)
    
    # Correctly build the bounding box in the API's expected format [lat, lon]
    bbox_api <- list(
      list(
  ...
  # The rest of the function remains largely the same, but uses bbox_api
  # and adds a local filtering step inside the onMessage handler
  ...
  start_connection <- function() {
    ...
    ws$onMessage(function(event) {
      ...
      if (!is.null(decoded)) {
        ...
        # Proceed only if the point is within the polygon (or if no polygon was provided)
        if (is_in_poly) {
